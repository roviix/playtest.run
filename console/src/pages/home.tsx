  ...
  return (
    <>
      <header class="workspace-head">
        <h1>我的作品</h1>
        <span class="workspace-count">{data.length} 个作品</span>
      </header>
      <ul class="wall">
        {data.map((site) => (
          <li key={site.slug}>
...
function factOf(site: Site): string {
  const listing = site.listing;
  if (listing?.seats) return `${listing.joined ?? 0} / ${listing.seats} 位`;
  const remaining = left(site.expires_at);
  if (remaining) return remaining;
  if (listing && (listing.followers ?? 0) > 0) return `${listing.followers} 人关注`;
...
function Start({ me }: { me: Me | null }) {
  ...
  return (
    <section class="start">
      ...
      <h1>把能玩的版本发出去</h1>
      <p class="start-lead">在作品目录里跑一条命令。拿到链接和二维码；需要图片时再下载邀请卡。谁来玩过，回到这里看。</p>
      <ol class="start-commands">
        ...
        <li>
          <code>playtest ./dist --seats 10</code>
          <span>放到广场上，找 10 位试玩</span>
        </li>
      </ol>
      <a class="docs-context-link" href={href({ name: "docs", section: "start" })}>从安装到第一次发布，查看使用文档 →</a>
