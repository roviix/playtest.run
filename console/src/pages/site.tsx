...
export function SitePage({
  ...
  return (
    <>
      <Identity site={site} />
      <Now site={site} results={results.data} />
...
function Identity({ site }: { site: Site }) {
  ...
  if (listing && listing.followers > 0) chips.push({ text: `${listing.followers} 人关注`, tone: "plain" });
  if (listing?.seats) chips.push({ text: `${listing.joined} / ${listing.seats} 位`, tone: "plain" });
  ...
  return (
    <header class="identity">
      ...
      <div class="identity-body">
        <h1>{site.title}</h1>
        <p class="link-row">
          <a class="mono player-link" href={site.url} target="_blank" rel="noreferrer">
 ...
 */
function Now({ site, results }: { site: Site; results: SiteResults | undefined }) {
  const current = site.current_version;
  ...
  const version: VersionResults | undefined = results?.versions.find((one) => one.version === current);
