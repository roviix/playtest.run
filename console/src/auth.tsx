...
export function LoginDialog({ request, onClose, onAuthenticated }: {
  ...
  return <dialog ref={dialog} class="account-dialog login-dialog" aria-labelledby="login-title" onClose={onClose} onCancel={() => { generation.current++; }} onClick={(event) => {
    ...
    <div class="notice-head">
      ...
      <button type="button" aria-label="关闭登录" onClick={() => dialog.current?.close()}>×</button>
    </div>
    ...
    <div class="login-token">
      ...
      {tokenMode ? <form id="token-login-form" class="notice-form" onSubmit={submit} aria-busy={busy}>
        <label>访问令牌<input ref={input} type="password" name="token" autoComplete="off" spellcheck={false} value={value} placeholder="粘贴访问令牌" required disabled={busy} aria-invalid={error ? true : undefined} aria-describedby={error ? "login-error" : undefined} onInput={(event) => { setValue(event.currentTarget.value); setError(""); }} /></label>
        <button type="submit" disabled={!value.trim() || busy}>{busy ? "正在验证…" : "登录"}</button>
        <a class="login-help" href={href({ name: "docs", section: "start" })} onClick={onClose}>在哪里找到令牌？</a>
      </form> : null}
    </div>
