...
5. **~~Postgres 而非 SQLite~~ → 先把真正的缺陷修掉，驱动不换（2026-09-11 改）。** 原来的理由是「已经是并发写入」。真正的缺陷是那把**单连接互斥锁**，不是 SQLite：WAL 模式加一个连接池就能解决，而换驱动要把 1,700 行 rusqlite 与 6,100 行测试全改一遍，产出零个用户可见的变化。更硬的一条：这台开发机上没有 Postgres 也没有跑着的 Docker，换过去等于写一份**跑不了也测不了**的代码——那正是「半残品」的定义。所以：保留 rusqlite，把单锁换成 WAL 连接池，`db.rs` 按聚合切分（这本来就是计划里的事），让将来真要换驱动时是一次机械替换。目标规模是几千个回访试玩者（DESIGN §3.9），SQLite 在这个量级上绰绰有余。
6. **对象存储的目录名仍是 `sites/`，不改成 `projects/`。** 用户看得见的地方（API 路径、CLI 用词、文档、界面）该叫作品就叫作品；盘上的目录名玩家和开发者都看不见，改它要给已经在线的作品做一次搬家，风险不为零而收益为零。
7. **投影是三个不是四个。** 原计划里 `ProjectCard`（控制台作品墙）和 `ProjectPublic`（`plaza.json` 里的一项）是两个类型，写的时候发现它们是同一张卡：广场和控制台用同一个类型，同一个作品在两个地方才会说同一句话。合成一个更符合「一个原语」。
8. **退出码不改。** §3.1 原来列了一套新的（0/1/2/3/4/5/6），是在没看现有实现的情况下写的；现有的那套（0 成功、1 意外、2 用法、3 要登录、4 网络、5 服务端、6 输入、7 配额）分得更细、已经在 `--help` 里写着、也已经随 v0.2.0 发出去了。换它是纯破坏。只去掉了 `not_implemented` 这一种——CLI 里已经没有「还没做好」的分支了。
9. **`--seek` 并入 `--note`。** 「想让人看什么」和「这版改了什么」对开发者是同一句话，对玩家也是同一句话（门禁页上、通知里）；三个 140 字文本变两个。
10. **不做自定义域名。** §1.2 里没有一个时刻需要它：试玩链接活几天到几周，引擎导出物用绝对路径也不在意域名；它要 Caddy 按需签证书与域名验证，是一整块新面。here.now 做它因为它是托管商。Pro 卖的是自选名字，不是域名。
11. **不做团队 Workspace，做 3 位只读 reviewer。** 发行商、老师、评委看结果是真实场景；共同发布不是——发布是一条命令，谁敲都一样。
12. **不做 Drive / Site Data / Proxy。** Site Data 与 Proxy 是半个后端，触到「不运行用户代码」的边；有后端需求的人走 `--backend`，这正是我们比 here.now 多出来的那条路。
13. **双语进 v1.0。** 引擎社区、Game Jam、AI 工具的用户一半在英文世界；agent 读的文档必须英文；只中文等于放弃 `playtest mcp` 这条最大供给管道的一半。
14. **玩家域零脚本不变，控制台不追求零脚本。** 门禁页是信任凭证，快与干净是它的全部；控制台是登录后的工作台。
15. **验收是数字加记录，不是等待。** 每个里程碑收尾记一条 spike 是为了不虚报；没有任何一条设计决定挂在「验证之后再定」上。真机结果推翻某条决定时，改 DESIGN，不改本文的施工方式。
16. **低报能力和虚报一样要修。** MCP 的 `playtest_share_port` 硬写着「这条路还没上线，调了一定失败」，而 CLI 的隧道早有几百行集成测试；一条测试还把这句话钉住了。AGENTS 第 4 条通常被读成「不要吹」，它的另一半是「做到了要说」——助手照着那句话就不会用隧道，用户也就不知道有这条路。已改成 `playtest_share`：隧道放进后台，连上就回答。
